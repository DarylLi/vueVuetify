...
<!--     <v-layout>
      <v-flex xs12 lg12>
      <tableTools :dataLoaded='dataLoaded' :headers='tableHeaders'
 :desserts='tableDesserts'></tableTools>
      </v-flex>
    </v-layout> -->
  </v-app>
</template>
  ...
  methods: {
    ...
    creatCharts (idName) {
      ...
      dbDemo(formData).then(res => {
        ...
        this.initChart(idName)
      })      
    },
    renderTable(idName, obj) {
      let curCfg = this.sqlOption[idName].sql
      if (obj) {
        let timeFilter = obj.curTime ? this.$_.cloneDeep(this.tableTimeQuery[obj.curTime]) : [{"columnName":"日期类型","filterType":"[a,b]","values":[obj.date, obj.date2]}]
